  ...
  if (questions.length === 0) {
    return (
      <ThemedView style={[styles.container, { paddingTop: insets.top }]}>
        <ThemedText type="body">{t("lessons:noPracticeQuestions")}</ThemedText>
        <Button onPress={handleClose}>{t("common:goBack")}</Button>
      </ThemedView>
    );
  }
      ...
      >
        <View style={styles.completeContainer}>
          ...
          <ThemedText type="h2" style={styles.completeTitle}>
            {t("lessons:practiceComplete")}
          </ThemedText>
          <ThemedText
            type="body"
            style={[styles.completeSubtitle, { color: theme.textSecondary }]}
          >
            {t("lessons:youGotXOutOfY", { correct: correctCount, total: questions.length })}
          </ThemedText>
