...
export default function LessonsScreen() {
  ...
  return (
    ...
    >
      ...
      >
        <CategoryChip
          label={t("lessons:categories.all")}
          icon="grid"
        ...
        {CATEGORIES.map((cat) => (
          <CategoryChip
            key={cat.id}
            label={t(`lessons:categories.${cat.id}`)}
            icon={cat.icon}
      ...
      >
        ...
        {DIFFICULTIES.map((diff) => (
          <DifficultyChip
            key={diff.id}
            label={t(`lessons:difficulty.${diff.id}`)}
            color={diff.color}
